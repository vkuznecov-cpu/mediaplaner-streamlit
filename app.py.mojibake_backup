                ...
                if cs["type"] == "РЎРїСЂРѕСЃ (РїРѕ Р·Р°РїСЂРѕСЃР°Рј)":
                    ...
                    with custom_right:
                        ...
                        if st.button("РџСЂРёРјРµРЅРёС‚СЊ РєР°СЃС‚РѕРјРЅС‹Р№ РЅР°Р±РѕСЂ", key=f"cs_apply_custom_{set_id}", type="primary"):
                            ...
                            df_coeffs["РљРѕСЌС„. AOV"] = df_coeffs["РљРѕСЌС„."].astype(float)
                            df_coeffs = df_coeffs[["РќРѕРјРµСЂ РјРµСЃСЏС†Р°", "РњРµСЃСЏС†", "РљРѕСЌС„.", "РљРѕСЌС„. AOV"]]
                            cs["result"] = df_coeffs
...
with tab_plan:
    ...
    with st.expander("1.2 РќР”РЎ Рё РђРљ", expanded=True):
        ...
        ak_mode_label = st.selectbox(
            ...
            options=["РџСЂРѕС†РµРЅС‚ РїРѕ С€РєР°Р»Рµ", "Р¤РёРєСЃРёСЂРѕРІР°РЅРЅР°СЏ СЃСѓРјРјР° РІ РјРµСЃСЏС†", "Р¤РёРєСЃРёСЂРѕРІР°РЅРЅС‹Р№ РїСЂРѕС†РµРЅС‚"],
            index=0 if ak_mode == "percent" else 1 if ak_mode == "fixed" else 2,
            key="ak_mode_select",
        )
        ak_mode = (
            "percent" if ak_mode_label == "РџСЂРѕС†РµРЅС‚ РїРѕ С€РєР°Р»Рµ"
            else "fixed" if ak_mode_label == "Р¤РёРєСЃРёСЂРѕРІР°РЅРЅР°СЏ СЃСѓРјРјР° РІ РјРµСЃСЏС†"
            else "fixed_percent"
        )
            ...
            for _, r in df_res.iterrows():
                ...
                if metric_type == "custom":
                    ...
                    coeff_values[(cs_name, m, "aov")] = float(
                        r["РљРѕСЌС„. AOV"] if "РљРѕСЌС„. AOV" in df_res.columns else k
                    )
                else:
